...
void FloydWarshall_recurD(int n, int k, int i, int j, int size)
{
    if (size == m)
    {
    ...
    {
        size /= 2;
        cilk_spawn FloydWarshall_recurD(n, k, i, j, size);
        cilk_spawn FloydWarshall_recurD(n, k, i, j+size, size);
        cilk_spawn FloydWarshall_recurD(n, k, i+size, j, size);
        cilk_spawn FloydWarshall_recurD(n, k, i+size, j+size, size);
        cilk_sync;
        cilk_spawn FloydWarshall_recurD(n, k+size, i, j, size);
        cilk_spawn FloydWarshall_recurD(n, k+size, i, j+size, size);
        cilk_spawn FloydWarshall_recurD(n, k+size, i+size, j, size);
        cilk_spawn FloydWarshall_recurD(n, k+size, i+size, j+size, size);
        cilk_sync;
    }
}
...
void FloydWarshall_recurA(int n, int k, int i, int j, int size)
{
    ...
    else
    {
        ...
        cilk_spawn FloydWarshall_recurC(n, k, i+size, j, size);
        cilk_sync;
        FloydWarshall_recurD(n, k, i+size, j+size, size);
        FloydWarshall_recurA(n, k+size, i+size, j+size, size);
        cilk_spawn FloydWarshall_recurB(n, k+size, i+size, j, size);
        cilk_spawn FloydWarshall_recurC(n, k+size, i, j+size, size);
        cilk_sync;
        FloydWarshall_recurD(n, k+size, i, j, size);
    }
}
